...
ccrModelFile <- result$modelFile[result$exposureId == 1124300 & 
                                   result$outcomeId == 1 &
                                   result$analysisId == 1]
ccModel <- readRDS(file.path(outputFolder, ccrModelFile))
summary(ccrModel)
```
...
if (file.exists("s:/temp/vignetteCaseCrossover2/outcomeModelReference.rds")) {
  ...
  ccrModelFile <- result$modelFile[result$exposureId == 1124300 & 
                                     result$outcomeId == 1 &
                                     result$analysisId == 1]
  ccrModel <- readRDS(file.path(outputFolder, ccrModelFile))
  summary(ccrModel)
}
```
